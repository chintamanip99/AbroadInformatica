...
class MainContent extends Component{
  ...
  componentDidMount(){
    ...
    fetch('https://patilcm.pythonanywhere.com'+'/content/content',{
			method:'GET',
			headers:{
			}
    })
...
getUnique(arr, index) {
  ...
  next = (event) =>{
    ...
    .then(res => {
      ...
      this.setState(
        prevState=>({
          response:{results:[...prevState.response.results,...res.results],next:res.next}
        })
      );}else{
  ...
  previous = (event) =>{
    ...
    render(){
        return(
          ...
          <h4 align="center">Full Content</h4>
        ...
        {this.state.response.results.map(post => {
            ...
            <div class="card">
              ...
              <tr>
                {post.category2 ? 
                <td><h4>Category :<a href={"/1/"+post.category2.category1.id}>{post.category2.category1.image?<img src={post.category2.category1.image+'&Token='+this.state.token} height="80" width="80"/>:''}{post.category2.category1.title} <i class="arrow right"></i></a><a href={"/2/"+post.category2.id}>{post.category2.image?<img src={post.category2.image+'&Token='+this.state.token} height="80" width="80"/>:''} {post.category2.title} <i class="arrow right"></i></a></h4></td>
              :<p></p>  
              }
                </tr>
                <tr>
                {post.category1 ? 
                <td><h4>Category :<a href={"/1/"+post.category1.id}>{post.category1.image?<img src={post.category1.image+'&Token='+this.state.token} height="80" width="80"/>:''}{post.category1.title} <i class="arrow right"></i> </a></h4></td>
              :<p></p>  
              }
                </tr>
                <tr>
              ...
              <th>{post.description}</th>
                ...
                <tr>
                {post.image ? 
                <td><img src={post.image+'&Token='+this.state.token} class="responsive"/></td>
              :<p></p>  
              }
                </tr>
                ...
                <ReactPlayer  className='react-player' controls muted url={post.video+'&Token='+this.state.token} width="100%" height='100%'/>
                ...
                <ReactAudioPlayer
                  style={{width:'100%'}}
                  src={post.audio+'&Token='+this.state.token}
                  controls
                />
                </td>
                :  <p></p> 
              ...
              <tr width="25">
              <td width="33.3%"><i id={'like_'+post.id} className='fa fa-thumbs-up fa-4x'  onClick={this.state.token!='d814c5fbcc4ad9441d392088fd81c12e225a82d9'?()=>this.like(post.id,post.no_of_likes):()=>alert('Login Needed')}></i><span id={'liketext_'+post.id}></span></td><td width="33.3%"><i  class="fa fa-comment fa-4x" onClick={this.state.token!='d814c5fbcc4ad9441d392088fd81c12e225a82d9'?() => this.comment(post.id,'add_a_comment'):()=>alert('Login Needed')}></i></td><td width="33.3%"><i class="fa fa-share"></i></td>
              </tr>
                </table>
              </div>
